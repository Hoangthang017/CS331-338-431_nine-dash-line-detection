...
def load_model_vgg_config():
    base_path = './'
    config_output_filename = os.path.join(base_path, 'models/model_vgg_config.pickle')
...
def loadModel(C):
    ...
    model_rpn.load_weights('./models/model_frcnn_vgg.hdf5', by_name=True)
    model_classifier.load_weights('./models/model_frcnn_vgg.hdf5', by_name=True)
...
def predict(filepath): 
    ...
    name += 1
    if C == None:
        C = load_model_vgg_config()
    # Switch key value for class mapping
    class_mapping = C.class_mapping
    class_mapping = {v: k for k, v in class_mapping.items()}

    class_to_color = {class_mapping[v]: np.random.randint(0, 255, 3) for v in class_mapping}
    if model_rpn == None or model_classifier_only == None:
        model_rpn, model_classifier_only= loadModel(C)
    bbox_threshold = 0.7
...
def predict_Yolo(image_path):
    ...
    classes = None
    with open("./models/yolo.names", 'r') as f: # Thay đổi địa chỉ của fiell yolo.names tại đây
        classes = [line.strip() for line in f.readlines()]

    COLORS = np.random.uniform(0, 255, size=(len(classes), 3))
    #Random màu để vẽ bounding box
    net = cv2.dnn.readNet("./models/yolov4-custom_best.weights", "./models/yolov4-custom.cfg") #Thay đổi tên và địa chỉ file weight và file cfg tại đây
